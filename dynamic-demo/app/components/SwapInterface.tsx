...
export default function SwapInterface() {
  ...
  const fetchBalances = async (address: string, forceRefresh = false) => {
    ...
    try {
      const usdcBalance = await retryWithBackoff(() =>
        checkUSDCBalance(address as `0x${string}`)
      );
      const formattedUsdcBalance = formatUnits(usdcBalance, fromToken.decimals);

      const wethBalance = await retryWithBackoff(() =>
        checkWETHBalance(address as `0x${string}`)
      );
  ...
  const calculateQuote = async (amountIn: string) => {
    ...
    try {
      ...
      const quoteOutAmount = await quoteOut(
        amountInWei,
        USDC,
        WETH,
        fee,
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      let client = await primaryWallet.getWalletClient();
      ...
      client.account.signUserOperation = async (parameters) => {
        ...
        const signature = await client.signTypedData({
          domain: {
            ...
            verifyingContract: primaryWallet?.address as `0x${string}`,
            version: METAMASK_DELEGATION.version,
          },
          message: { ...packedUserOp, entryPoint: entryPoint07Address },
      ...
      >(client, {
        ...
        scw: {
          ...
          authorization: {
            ...
            account: client.account as PrivateKeyAccount,
          },
          entryPoint: {
            abi: entryPoint07Abi,
      ...
      const bal = await checkUSDCBalance(address as `0x${string}`);
      if (bal < amountIn) {
      ...
      const minOut =
        (out * (BigInt(10_000) - BigInt(SLIPPAGE_BPS))) / BigInt(10_000);
      console.log(
        `Quote: ${out.toString()} wei WETH; minOut (${SLIPPAGE_BPS} bps): ${minOut.toString()}`
      );

      const { allowanceData, swapData } = await getAllowanceAndSwapData(
        amountIn,
        fee,
        minOut,
        address as `0x${string}`
      ...
      const signature = await client.account.signUserOperation(
        preparedCalls?.signatureRequest.data as UserOperation
      );

      const results = await smartWalletClient?.sendPreparedCalls({
      ...
      await fetchBalances(address as `0x${string}`, true);
            ...
            <div className="bg-card rounded-xl shadow-lg border border-border p-6">
              <div className="text-center mb-8">
                ...
                <p className="text-muted-foreground text-sm">
                  Exchange USDC for WETH with zero gas fees
                </p>
              </div>
              ...
              {balanceError && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={() =>
                        userAddress && fetchBalances(userAddress, true)
                      }
                ...
                <div className="bg-muted rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      ...
                      {isLoadingBalances
                        ? "Loading..."
                        : formatBalance(toToken.balance, 10)}{" "}
                      {toToken.symbol}
                    </span>
                  </div>
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 relative">
                      <input
                        ...
                        readOnly
                        placeholder={
                          isCalculatingQuote ? "Calculating..." : "0.0"
                        }
                        className="w-full bg-transparent text-2xl font-semibold text-card-foreground placeholder-muted-foreground focus:outline-none"
                      />
                      {isCalculatingQuote && (
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-card-foreground">
                        {isCalculatingQuote
                          ? "Calculating..."
                          : quoteRate > 0
                          ? `1 USDC = ${quoteRate.toFixed(6)} WETH`
                          : "Enter amount"}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-muted rounded-lg px-3 py-2 flex items-center justify-between">
                    ...
                    {userAddress && (
                      <button
                        onClick={() =>
                          handleCopyToClipboard(userAddress, "smart")
                        }
